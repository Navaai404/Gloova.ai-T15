...
export const Chat: React.FC = () => {
  ...
  const handleSend = async () => {
    ...
    if (!canChat) {
        alert("Você não possui tokens de chat suficientes. Por favor, recarregue sua carteira.");
        navigate('/profile');
        return;
    }
    ...
    try {
      ...
      // Monetization Logic: Calculate cost based on length
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50">
      <div className="bg-white p-4 shadow-sm border-b border-slate-100 sticky top-0 z-10 flex justify-between items-center">
        ...
        {!canChat && (
            <button onClick={() => navigate('/profile')} className="text-xs bg-red-100 text-red-600 px-2 py-1 rounded-lg font-bold flex items-center gap-1">
      ...
      <div className="p-4 bg-white border-t border-slate-200 fixed bottom-16 left-0 right-0">
        <div className="flex gap-2 max-w-md mx-auto">
          <input
            ...
            disabled={!canChat}
            className="flex-1 bg-slate-100 border-0 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all outline-none disabled:opacity-50"
          />
          <button 
            onClick={handleSend}
