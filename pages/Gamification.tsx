...
export const Gamification: React.FC = () => {
  ...
  const loadData = async () => {
    try {
        // 1. Load Local User & Refresh from DB
        ...
        const { data: topUsers, error } = await supabase
            .from('profiles')
            .select('name, points, id, subscription_tier') 
  ...
  useEffect(() => {
    ...
    // REALTIME LISTENER: Escuta mudanças nos pontos para atualizar o ranking na hora
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
           // Se o update for relevante (pontos mudaram), recarrega o ranking
           if (payload.new && (payload.new as any).points !== (payload.old as any).points) {
               console.log("Pontos atualizados! Recarregando ranking...");
               loadData();
           }
        }
      )
        ...
        <div className="flex justify-center items-end gap-4 py-4 min-h-[140px]">
            {top2 && (
                <div className="flex flex-col items-center">
                    <div className="w-10 h-10 rounded-full border-2 border-slate-200 overflow-hidden mb-2 bg-slate-100 flex items-center justify-center text-xs font-bold text-slate-400 uppercase">{top2.name?.[0] || '?'}</div>
                    <div className="flex flex-col items-center w-16"><div className="w-full bg-slate-200 rounded-t-2xl h-16 relative flex justify-center pt-2"><span className="font-bold text-slate-500">2</span></div><span className="text-[10px] font-bold text-slate-600 mt-1 truncate w-full text-center">{top2.isMe ? 'Você' : top2.name?.split(' ')[0]}</span></div>
                </div>
            )}
            {top1 && (
                <div className="flex flex-col items-center relative -top-2">
                    ...
                    <div className="w-12 h-12 rounded-full border-2 border-yellow-400 overflow-hidden mb-2 shadow-lg shadow-yellow-200 bg-yellow-50 flex items-center justify-center text-sm font-bold text-yellow-600 uppercase">{top1.name?.[0] || '?'}</div>
                    <div className="flex flex-col items-center w-20"><div className="w-full bg-gradient-to-b from-yellow-300 to-yellow-400 rounded-t-2xl h-24 relative flex justify-center pt-2 shadow-lg shadow-yellow-200"><span className="font-bold text-white text-xl">1</span></div><span className="text-[10px] font-bold text-yellow-600 mt-1 truncate w-full text-center">{top1.isMe ? 'Você' : top1.name?.split(' ')[0]}</span></div>
                </div>
            )}
            {top3 && (
                <div className="flex flex-col items-center">
                    <div className="w-10 h-10 rounded-full border-2 border-amber-200 overflow-hidden mb-2 bg-amber-50 flex items-center justify-center text-xs font-bold text-amber-700 uppercase">{top3.name?.[0] || '?'}</div>
                    <div className="flex flex-col items-center w-16"><div className="w-full bg-amber-200 rounded-t-2xl h-12 relative flex justify-center pt-2"><span className="font-bold text-amber-800">3</span></div><span className="text-[10px] font-bold text-slate-600 mt-1 truncate w-full text-center">{top3.isMe ? 'Você' : top3.name?.split(' ')[0]}</span></div>
                </div>
            )}
        </div>
      )}
